...
class Student_attendance extends MY_Controller {
	...
	function show_search($kStudent = NULL)
	{
		...
		$data ['target'] = "student_attendance/search";
		if ($this->input->get ( "ajax" )) {
			$this->load->view ( $data ['target'], $data );
		} else {
			$this->load->view ( "page/index", $data );
	...
	function complete($date, $kTeach)
	{
		...
		$message = $this->load->view ( "student_attendance/checklist/email", $data, TRUE );
	...
	function truancy_notification($record, $subtype = FALSE)
	{
		if (($subtype == "Unexcused" && $record->total > UNEXCUSED_THRESHOLD) || $record->total > TRUANCY_THRESHOLD) {
			...
			$body ['link'] = sprintf ( "You can view %s's record <a href='%s'>here.</a>", $record->stuNickname, site_url ( "student_attendance/search/$record->kStudent?startDate=$startDate" ) );
	...
	function _checklist_buttons($date, $kStudent, $kTeach, $options = array())
	{
		if (array_key_exists ( "kAttendance", $options ) && $options ['kAttendance'] != NULL) {
			...
			$buttons [] = array (
					...
					"href" => base_url ( "student_attendance/absent?date=$date&kStudent=$kStudent&attendType=Tardy" ),
			...
			$buttons [] = array (
					...
					"href" => base_url ( "student_attendance/absent?date=$date&kStudent=$kStudent&attendType=Absent" ),
