...
class Student extends MY_Controller
{
    ...
    function view ()
    {
        ...
        $data["grade_preferences"] = $this->preference->get_all($kStudent,
                $options);
    ...
    function create ()
    {
        ...
        $data["teacherPairs"] = get_keyed_pairs($teachers,
                array(
                        "kTeach",
                        "teacher"
                ));
        ...
        if ($kStudent) {
            ...
            if (empty($student->stuEmail) &&
                     get_current_grade($student->baseGrade, $student->baseYear,
                            get_current_year()) > 2) {
                $student->stuEmail = $this->generate_email($kStudent,
                        $student->stuNickname, $student->stuLast);
            }
            $humanitiesTeachers = $this->teacher_model->get_for_subject(
                    "Humanities");
            $data["humanitiesTeachers"] = get_keyed_pairs($humanitiesTeachers,
                    array(
                            "kTeach",
                            "teacherName"
                    ));
            ...
            $data["action"] = "update";
            $grades = $this->menu_model->get_pairs("grade");
            $data["gradePairs"] = get_keyed_pairs($grades,
                    array(
                            "value",
                            "label"
                    ));
            $gender = $this->menu_model->get_pairs("gender");
            $data["genderPairs"] = get_keyed_pairs($gender,
                    array(
                            "value",
                            "label"
                    ));
    ...
    function advanced_search ()
    {
        ...
        $data["students"] = $this->student_model->advanced_find($year, $grades,
                $hasNeeds, $includeFormerStudents, $sorting, $stuGroup);
        $data["title"] = "Student List";
    ...
    function get_class_lists($grade_start = NULL, $grade_end = NULL){
    	$teachers = $this->teacher_model->get_all(array("roles"=>2,"gradeSort"=>1,"gradeRange"=>array("gradeStart"=>$grade_start,"gradeEnd"=>$grade_end)));
    ...
    function valid_email ($kStudent = FALSE, $stuEmail = FALSE)
    {
        ...
        if ($stuEmail) {
            $count = $this->student_model->count("stuEmail", $stuEmail,
                    array(
                            "kStudent" => $kStudent
                    ));
    ...
    function update_all_emails ()
    {
        ...
        foreach ($students as $student) {
            $this->student_model->update_value($student->kStudent,
                    array(
                            "stuEmail" => $this->generate_email(
                                    $student->kStudent, $student->stuNickname)
                    ));
            $this->student_model->update_value($student->kStudent,
                    array(
                            "stuEmailPassword" => $this->generate_password(
                                    $student->stuFirst, $student->stuLast)
                    ));
        }
    }
