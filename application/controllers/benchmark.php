...
class Benchmark extends MY_Controller
{
	...
	function show_search()
	{
		...
		$data["subject_list"] = get_keyed_pairs($data["subjects"], array("subject","subject"),FALSE);
		$data["yearStart"] = get_current_year();
		$data["yearEnd"] = $data["yearStart"] + 1;
		$teacher = $this->teacher_model->get($kTeach, array("gradeStart","gradeEnd"));
		$data["gradeStart"] = $teacher->gradeStart;
		$data["gradeEnd"] = $teacher->gradeEnd;
		$this->load->view("benchmark/search", $data);
