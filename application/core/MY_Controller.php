...
class MY_Controller extends CI_Controller
{
    ...
    function __construct ()
    {
        ...
        if (! is_logged_in($this->session->all_userdata())) {
        	...
        	define('TRUANCY_THRESHOLD', $this->config_model->get_by_key("truancy_threshold")?->config_value ?? 0);
        	define('UNEXCUSED_THRESHOLD', $this->config_model->get_by_key("unexcused_threshold")?->config_value ?? 0);
        	define('ILLNESS_THRESHOLD', $this->config_model->get_by_key("illness_threshold")?->config_value ?? 0);
