...
$year = get_value($category, "year", get_cookie("year"));
$yearEnd = $year + 1;
?>
<tr <?php echo($action == "insert" ? "id='tr-teach_$kTeach'" : "id='category-$category->kCategory'"); ?> >
    ...
    <td><input type="text" name="gradeStart"
               ...
               value="<?php echo get_value($category, "gradeStart", get_cookie("assignment_grade_start")); ?>"
               size="2"/></td>
    <td><input type="text" name="gradeEnd" class="<?php echo $action; ?>-gradeEnd"
               id="gradeEnd_<?php echo get_value($category, "kCategory", "new"); ?>"
               value="<?php echo get_value($category, "gradeEnd", get_cookie("assignment_grade_end")); ?>" size="2"/>
    </td>
    <td>
    ...
    <td>
        <?php echo form_dropdown("year", get_year_list(FALSE, TRUE), $year, sprintf("id='year_%s' data-type='year' data-action='new' class='year %s-year'", get_value($category, "kCategory"), $action)); ?>
        -<input type="text" name="yearEnd" id="yearEnd"
                readonly value="<?php echo $yearEnd; ?>" size="4"/></td>

    <td><span class="button category-<?php echo $action; ?>" data-action="<?php echo $action; ?>"
              data-teacher='<?php echo $kTeach; ?>'
              data-category="<?php echo get_value($category, "kCategory", "new"); ?>"
