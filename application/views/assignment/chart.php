	...
	if (! empty ( $grades )) {
		...
		foreach ( $grades as $grade ) {
			if ($current_student != $grade->kStudent) {
				...
				$rows [$grade->kStudent] ["delete"] = sprintf ( "<td class='grade-delete-row'><span class='student delete button' data-teacher='%s' data-student='%s' data-term='%s' data-year='%s' title='Delete the entire row'>Delete</span></td>", $kTeach, $grade->kStudent, $header->term, $header->year  );
