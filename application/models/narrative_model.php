...
class Narrative_model extends CI_Model
{
	 ...
	 * @param  $narrYear
	 *
	 * produces a result of all the students within a grade range for whom no narrative has been written by a given teacher for the term year and subject
	 */
	...
	function update_text($kNarrative, $narrText, $backup = FALSE)
	{
		...
		$this->db->where("kNarrative", $kNarrative);
		$data = array("narrText"=>$narrText, "recModifier" => $this->session->userdata("userID"),"recModified" => mysql_timestamp());
