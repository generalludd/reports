 ...
 * @param s $format
 *          string
 *          description: this shouldn't be in this file, but I didn't want to
 *   create a new file with general formatting tools yet. idea courtesy:
...
/**
 * Format a range of two dates, if the second is null or the same as the first,
 * show just one date.
 *
...
/**
 * @TODO consider a system preference or constant declaration for the cutoff
 *   month.
 */
...
/**
 * @TODO this may be something that could be modified with a system preference
 *   using the "config" table for term names, durations and cutoffs. for now it
 *   is hard-coded.
 *
...
function get_term_menu($id, $currentTerm = NULL, $initial_blank = FALSE, $options = [], $is_required = FALSE) {
  $terms = [
    "Mid-Year",
    "Year-End",
  ];
 ...
 */
function get_keyed_pairs($list, $pairs, $initialBlank = NULL, $other = NULL, $alternate = []) {
  $output = [];
  if ($initialBlank) {
    $output [""] = "";
  }
...
function create_grade_checklist($start = 0, $limit = 8, $name = "grades", $grade_cookie = []) {
  $id = $name;
  $name = $name . "[]";
...
function format_name($firstName, $lastName, $nickname = NULL, $separator = NULL) {
  ...
  if ($informal) {
    ...
    if ($nickname != NULL and $nickname != $firstName) {
 ...
 *   have no comma, list with 3 or more have commas and final conjunction)
 *
 * @return string
 */
...
function format_table($data, $header = [], $options = []) {
  $table = [];
  $table_class = "";
